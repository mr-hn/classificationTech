...
63% of the Kaggle users reported to use GLM at work, compared to 46% who
...
Passing `attrition` to `strata` ensures that the 0's and 1's of the response are 
equally distributed.
```{r}
...
right-hand-side are assigned a coefficient and added up. The coefficients are calculated
minimizing negative log-likelihood estimation, and the sum of the predictors gives the log-odds
or `logit` equivalent of the event occurring.

<br>
<center>$$\operatorname{logit} p = \ln \left( \frac{p}{1 - p} \right) = \beta_0 + \beta_1 x $$ </center><br>
The probability is then calculated by exponentiating both sides and p will now a
...
Unit change in performance rating 1 to 2 increases the probability by about 0.33 
...
have larger influence in estimating $y$. The ROC curve is plotted below.
...
while `r glm_eval_thres$matrix[2,1]` who did quit were not identified correct.
```{r}
glm_eval_thres <- conf_matrix(glm_predict,
                              threshold = glm_thres_tune %>%
                                dplyr::select("threshold") %>% head(1) %>% .[[1]])
...
improved. Mathematically, this works by adding a penalty parameter $P$ to the 
log-likelihood estimator of $\beta$. Higher the penalty, lower the coefficients calculated.
The penalty is further controlled by the $\lambda$.

<br><center>$$minimize \{L(\beta) + \lambda P \} $$</center><br>

Now, the reason they work is because $P_{ridge} = |\beta^2|$ and $P_{lasso} = |\beta|$.
That is, in case of ridge, owing to the minimization, the addition of $\beta^2$ forces
the forcing the magnitude of the coefficient to be as small as possible. In lasso,
$\beta$ can straight up cancel the $-\beta$, forcing coefficients all the way to zero
if they don't help the model. This property of regularization is used as a variable 
selection method in modelling.<br><br>

The implementation requires an $\alpha$ parameter that needs to be passed. The 
$\lambda$ is calculated by the function by cross-validating on the training
data. Setting $\alpha$ to 0 makes it a ridge regression, while 1 makes it a lasso. 
Elasticnet is a combination of both lasso and ridge, with $\alpha$ between 0 to 1.
The full formula is

<br><center>$$minimize \{L(\beta) + \lambda[P_{ridge} (1-\alpha) + P_{lasso} (\alpha)] \} $$</center><br>

One thing to note is that setting $\lambda = 0$ makes it a general regression while
setting $\lambda = \infty$ makes all coefficients zero.

## Basic Lasso

A basic lasso regression with $\alpha$ set to 1 is implemented below. As mentioned 
earlier, through cross validation on the training dataset, the $\lambda$ that
minimizes the errors is estimated by the function This is then passed while
predicting on the test dataset. `lambda.min` is the value of lambda with the lowest
cross-validated error. `nfolds` is set at the default value of 10.
```{r fig.height=4, fig.width= 4}
set.seed(1804)
...
a little overfit model. Increasing it a little further to ~0.01 with ~22 terminal
nodes is more optimal. The actual tree for the this CP is visualized below. 
```{r out.width="100%", fig.height=4}
set.seed(1804)
rpart_model <- rpart(attrition ~ ., data_train, method = "class",
                     parms = list(split = "information", 
...
maxdepth and CP. Since CP if 0.01 did not give great results, it's reduced in 
steps of 0.001 to 0.001. The maxdepth is increased to a range between 8 and 15,
from the current 9.
```{r}
...
a large range of `min.split` indicating that shorter trees/ less complex models have
better prediction accuracty. Here's a look at the model with the best performance.
...
(less complex) is fitting better on the testing data.
</p></div>
...
What if data is randomly sampled into multiple chunks and trees are grown over each chunk
and then averaged? That's the idea behind <b>b</b>ootstrap <b>agg</b>regat<b>ing</b>.
The concept is simple and it solves decision tree's drawback of poor adaptability on
new data. Below is a basic bagging model. We see an immediate jump in the area under the curve.
```{r fig.height=4, fig.width= 4}
set.seed(1804)
...
ranger_tuning %>% arrange(desc(auc)) %>% head(20) %>% 
...
0.7, while cost of bagging went up to 0.95. This leads us to conclude that this random
forest is a better model than bagging.
```{r}
...
of methods of measuring this importance, and both make use of the out-of-bag error.<br>
...
data.frame(num_trees = 1:gbm_model$n.trees,
  ...
  labs(x = "Number of Iterations", y = "Bernoulli Deviance") + theme_light() +
...
work with 1000 trees since lower shrinkage values are being tuned for. Another
...
```
The 36-model grid took 21 minutes trun and this is one of the drawbacks of GBM. They are 
...
a huge leap in performance over a decision tree, and we do observe the 
gradient descent learning, but random forest performed better. <br><br>
...
<br><br>
<br><br>
Source files can be found <u><a href="https://github.com/mr-hn/classificationTech"target="_blank">here</a></u>.
